...
DATA_TAR = BASE / "Dataset" / "mcg_dataset.tar"
GSHEET_PATH = BASE / "Documentation" / "Testing.gsheet"
OUTPUT_DIR = (BASE / "Codes" / "Testing" /
              "3-channel (Mel, CQT, Gamma) + VGGNet19" / "Outputs")

SHEET_TAB = "3-channel (Mel, CQT, Gamma) + VGGNet19"
GSHEET_TITLE_FALLBACK = "Testing"
CREDENTIALS_JSON = "credentials.json"

LOCAL_DATA = Path("/content/mcg_data")
...
def stage_dataset():
    ...
    t0 = time.time()
    sh(f'cp "{DATA_TAR}" /content/_mcg.tar')
    sh(f'tar -xf /content/_mcg.tar -C "{LOCAL_DATA}"')
    sh("rm -f /content/_mcg.tar")
...
def build_model(num_classes):
    """torchvision VGG19 (no BatchNorm) with a fresh 5-way classifier head."""
